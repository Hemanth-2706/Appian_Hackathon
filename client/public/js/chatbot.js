...
function sendMessage() {
	...
	// Fetch the latest gemini summary from the backend
	fetch("/process-recommendations", {
		method: "POST",
		headers: {
			"Content-Type": "application/json",
		},
		...
		.then((data) => {
			if (data.geminiSummary) {
				appendMessage(
					`Here is what I understood: ${data.geminiSummary}
Please click on show recommendations for results or continue conversation for any changes.`,
		...
		.catch((err) => {
			console.error("Gemini summary fetch error:", err);
			appendMessage("Sorry, I couldn't get the session summary.", "bot");
		});
}
