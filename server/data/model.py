...
class FashionRecommender:
    def __init__(self, embeddings_dir="./", output_dir="./"):
        ...
        self.session_history = []  # Track user input prompts
...
"""
    ...
    def recommend(self, img=None, prompt=None, k=K):
        ...
        # Append user prompt to session_history if present
        if has_txt and prompt:
        ...
        if has_txt or has_img:
            ...
            # Get session summary to use as context
            session_summary = self.get_session_summary()
            # Get complementary categories using session summary instead of user prompt
            cats = self.ask_complements_local(caption, session_summary if has_txt else "")
    ...
    def get_session_summary(self):
        """Generate a session summary using Gemini based on session_history."""
        if not self.session_history:
            logger.info("Session history is empty. No summary to generate.")
            return "No session history to summarize."
        prompt = (
            "You are an assistant. Here is the conversation history of a user session:\n"
            + "\n".join(f"- {entry}" for entry in self.session_history)
            + "\n\nPlease provide a concise summary of this session, highlighting the main topics, requests, and any preferences or patterns you notice."
        )
