...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:5003"],  # Your Node.js server
...
async def process_recommendations(request: RecommendationRequest):
    ...
    try:
        ...
        session_summary = recommender.get_session_summary()
        gemini_message = f"Here is what I understood: {session_summary}\nPlease click on show recommendations for results or continue conversation for any changes."
...
@app.post("/clear-session-history")
async def clear_session_history():
    logger.info("=== Clearing session history and summary in FashionRecommender ===")
    recommender.session_history = []
    logger.info("Session history cleared.")
    return JSONResponse(content={"success": True, "message": "Session history cleared."}, media_type="application/json")
...
if __name__ == "__main__":
    ...
    logger.info("Server will be available at http://0.0.0.0:5002")
